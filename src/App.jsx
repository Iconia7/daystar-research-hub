...
/* --- 1. DEFINE SUB-COMPONENTS FIRST (FIXES BLANK PAGE ERROR) --- */
...
const ResearchersView = ({ data }) => (
  <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto h-full pb-20 animate-fade-in">
    {data.map(node => (
      <div key={node.id} className="p-5 bg-[#1e293b]/30 border border-white/5 rounded-2xl hover:bg-[#1e293b]/50 transition-all group">
...
const ProjectsView = ({ data }) => (
    <div className="p-8 space-y-4 overflow-y-auto h-full pb-20 animate-fade-in">
      {data.map(node => (
        <div key={node.id} className="p-6 bg-[#1e293b]/30 border border-white/5 rounded-2xl flex items-center justify-between hover:border-purple-500/30 transition-all">
           <div className="flex items-center gap-4">
...
function App() {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#25aae1] text-slate-100 overflow-hidden font-sans selection:bg-purple-500/30">
      ...
      {/* LEFT SIDEBAR */}
      <div className="w-72 flex-none border-r border-white/5 bg-[#25aae1] flex flex-col z-20 shadow-2xl">
        <div className="p-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight tracking-tight">Daystar Research Hub</h1>
              <p className="text-[10px] text-white font-medium tracking-widest">INTELLIGENCE GRAPH</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 mt-2 flex-1 overflow-y-auto">
           <div className="flex items-center gap-2 mb-4">
             ...
             <h3 className="text-xs font-bold text-white uppercase tracking-widest">Opportunities</h3>
           </div>
      ...
      <div className="flex-1 flex flex-col relative bg-[#25aae1]">
        ...
        {/* Top Navigation */}
        <div className="h-16 border-b border-white/5 bg-[#25aae1]/50 backdrop-blur-md flex items-center justify-between px-6 z-10">
           <div className="flex items-center bg-white/50 px-4 py-2 rounded-xl border border-white/10 w-96 text-sm focus-within:border-purple-500/50 focus-within:bg-[#1e293b] transition-all duration-300">
              <Search size={16} className="mr-3 text-white" />
              <input type="text" placeholder="Search researchers, topics, methods..." className="bg-transparent border-none outline-none w-full placeholder-slate-500 text-slate-200" />
           </div>
        ...
        {/* Feature Tabs */}
        <div className="px-6 pt-4 pb-0 bg-white border-b border-white/5">
        ...
        <div className="flex-1 relative overflow-hidden">
           ...
           {activeTab === 'graph' && (
              <>
                ...
                {/* Floating Overlay: Interactive Guide */}
                <div className="absolute top-6 left-6 bg-white/90 backdrop-blur border border-white/10 p-5 rounded-2xl shadow-2xl max-w-xs animate-fade-in pointer-events-none select-none">
                ...
                <div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur border border-white/10 p-4 rounded-xl shadow-2xl">
           ...
           {/* VIEW 3: PROJECTS */}
           {activeTab === 'projects' && <ProjectsView data={data.nodes.filter(n => n.group === 'project' || n.department === 'Project')} />}
      ...
      {selectedNode && (
        <div className="w-96 border-l border-white/5 bg-[#0f1014] flex flex-col shadow-2xl animate-slide-in-right z-30 absolute top-0 bottom-0 right-0">
...
const FilterDropdown = ({ label }) => ( <button className="flex items-center gap-2 px-4 py-2 text-xs font-medium bg-[#1e293b]/50 border border-white/10 rounded-xl text-slate-300 hover:bg-[#1e293b] hover:border-white/20 transition-all">{label} <ChevronDown size={14} /></button> );
const DashboardCard = ({ label, value, color = "text-black" }) => ( <div className="bg-white p-4 rounded-xl border border-white/5"><div className={`text-2xl font-bold ${color}`}>{value}</div><div className="text-[10px] text-slate-500 font-medium uppercase mt-1">{label}</div></div> );
const SuggestionCard = ({ initials1, initials2, names, topic, score }) => ( <div className="p-4 bg-white rounded-xl border border-white/5 hover:border-purple-500/30 transition-all cursor-pointer group"><div className="flex items-center justify-between mb-3"><div className="flex -space-x-2"><div className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center text-xs font-bold ring-2 ring-[#0f1014]">{initials1}</div><div className="h-8 w-8 rounded-full bg-lime-500 flex items-center justify-center text-xs font-bold text-black ring-2 ring-[#0f1014]">{initials2}</div></div><Sparkles size={14} className="text-purple-500 group-hover:animate-pulse" /></div><p className="text-sm font-semibold text-black">{names}</p><p className="text-[11px] text-slate-500">{topic}</p><div className="mt-2 text-[10px] bg-purple-500/10 text-purple-500 inline-block px-2 py-1 rounded border border-purple-500/20">{score} Match</div></div> );
